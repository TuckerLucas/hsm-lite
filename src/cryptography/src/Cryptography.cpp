...
optional<vector<uint8_t>> Cryptography::aes256Encrypt(const Key& key, const vector<uint8_t>& plainText, AesMode aesMode, vector<uint8_t> iv)
{
    return aes256Crypt(key, plainText, aesMode, CipherOperation::Encrypt, iv);
}

optional<vector<uint8_t>> Cryptography::aes256Decrypt(const Key& key, const vector<uint8_t>& cipherText, AesMode aesMode, vector<uint8_t> iv)
{
    return aes256Crypt(key, cipherText, aesMode, CipherOperation::Decrypt, iv);
}

optional<vector<uint8_t>> Cryptography::aes256Crypt(const Key& key, const vector<uint8_t>& input, AesMode aesMode, CipherOperation cipherOperation, vector<uint8_t> iv)
{
