...
{
    return aes256Crypt(key, plainText, aesMode, CipherOperation::Encrypt);
}

optional<vector<uint8_t>> Cryptography::aes256Decrypt(const Key& key, const vector<uint8_t>& cipherText, AesMode aesMode)
{
    return aes256Crypt(key, cipherText, aesMode, CipherOperation::Decrypt);
}

optional<vector<uint8_t>> Cryptography::aes256Crypt(const Key& key, const vector<uint8_t>& input, AesMode aesMode, CipherOperation cipherOperation)
{
    ...
    int initStatus = (cipherOperation == CipherOperation::Encrypt)
        ? EVP_EncryptInit_ex(ctx.get(), cipher, nullptr, key.data.data(), nullptr)
        : EVP_DecryptInit_ex(ctx.get(), cipher, nullptr, key.data.data(), nullptr);
